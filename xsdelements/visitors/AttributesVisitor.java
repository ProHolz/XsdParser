...
public abstract class AttributesVisitor extends XsdAnnotatedElementsVisitor {
	...
	public ISequence<XsdAttribute> getXsdAttributes() {
		return attributes
				.Where(attribute -> attribute instanceof ConcreteElement)
				.Where(attribute -> attribute.getElement() instanceof  XsdAttribute)
			...
			public ISequence<XsdAttributeGroup> getXsdAttributeGroup() {
				return attributeGroups
						.Where(attributeGroup -> attributeGroup instanceof ConcreteElement)
					...
					public void replaceUnsolvedAttributes(NamedConcreteElement element){
						if (element.getElement() instanceof XsdAttributeGroup){
							...
							attributeGroups
									.Where(attributeGroup -> attributeGroup instanceof UnsolvedReference && XsdAbstractElement.compareReference(element, (UnsolvedReference) attributeGroup))
									.FirstOrDefault();
									if (assigned(referenceBase))
						...
						if (element.getElement() instanceof XsdAttribute ){
							ReferenceBase referenceBase =
							attributes
									.Where(attribute -> attribute instanceof UnsolvedReference && XsdAbstractElement.compareReference(element, (UnsolvedReference) attribute))
									.FirstOrDefault();
